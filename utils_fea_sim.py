...
import argparse
import sys
# given_preference = known feature total
def feature_distance(given_preference, userID, TopKTaxo, features):        
    ...
    for i in range(len(given_preference)): # given_preference list:  cluster list, type list, category list
        if i == 0:
    ...
    if len(given_preference) > 0:
        ...
        time, category, cluster, poi_type = target_item_features_list[-1].split(',') # target item features
...
#        print (feature_index_embedding.size())
