...
#        
        ...
        if A.purpose == 'pretrain':
            if len(numpy_list) > 5000:
                ...
                numpy_list = list()
        # end write
    print ('Random Location result:')
    sum_sr = 0.0
    for i in range(len(success_at_turn_list_location_random)):
        success_rate = success_at_turn_list_location_random[i]/A.endAt
        sum_sr += success_rate
        print ('success rate is {} at turn {}, sum is {}'.format(success_rate, i+1, sum_sr))   
    print ('Average turn is: ', total_turn/A.endAt +1)
    
    print ('\nRate Location result:')
    sum_sr = 0.0
    ...
    print ('\nItem result:')
    sum_sr = 0.0
    for i in range(len(success_at_turn_list_item)):
        success_rate = success_at_turn_list_item[i]/A.endAt
        sum_sr += success_rate
        print ('success rate is {} at turn {}, sum is {}'.format(success_rate, i+1, sum_sr))   
    print ('Average turn is: ', total_turn/A.endAt +1)
